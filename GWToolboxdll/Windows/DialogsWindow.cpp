...
void DialogsWindow::Draw(IDirect3DDevice9*)
{
    if (!visible) {
        return;
    }
    ...
    if (ImGui::Begin(Name(), GetVisiblePtr(), GetWinFlags())) {
        ...
        if (show_custom) {
            const int n_DialogsWindow = _countof(dialognames);
            static int dialogindex = 0;
            ImGui::PushItemWidth(-60.0f - ImGui::GetStyle().ItemInnerSpacing.x);
            ImGui::Combo("###dialogcombo", &dialogindex, dialognames, n_DialogsWindow);
            ImGui::PopItemWidth();
            ImGui::SameLine(0, ImGui::GetStyle().ItemInnerSpacing.x);
...
void DialogsWindow::DrawSettingsInternal() {
